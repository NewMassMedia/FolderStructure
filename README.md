...
- **`Plugins/`** — Unity가 예약한 **특수 폴더**입니다. 네이티브 플러그인(`.dll`/`.so`/`.aar`)과 플랫폼별 바이너리(`Plugins/Android`, `Plugins/iOS`)만 둡니다. 일반 C# 에셋을 여기 막 넣지 마세요.
- **`ThirdParty/`** — 에셋스토어 패키지, 외부 매니지드 라이브러리 등 **일반 외부 에셋**. 우리가 만든 것이 아닌 코드/에셋은 여기에 격리해 업데이트·교체를 쉽게 합니다.
...
| Skeletal/Character Mesh | `SK_` | `SK_CharacterA` |
...
- **`_Sandbox`는 릴리스 빌드 전에 반드시 정리**한다. 실험물이 빌드에 섞이지 않도록, 빌드 스크립트에서 제외하거나 씬/Addressables 그룹에 포함하지 않는다.
